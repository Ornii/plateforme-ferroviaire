...
void sendRailSwitch() {
    packet_to_send = 0;
    packet_to_send = packet_to_send | (GET_RAIL_SWITCH <<  1);
...
void loop() {
  delay(50); // nécessaire pour pas surcharger...
  ...
  if (hall_sensor_diverging_track_state_new != hall_sensor_diverging_track_state && hall_sensor_diverging_track_state == 0) {
      hall_sensor_diverging_track_state = 1;
  }

  if (hall_sensor_straight_track_state_new != hall_sensor_straight_track_state && hall_sensor_straight_track_state == 0) {
      hall_sensor_straight_track_state = 1;
  }

  if (hall_sensor_main_track_state_new != hall_sensor_main_track_state && hall_sensor_main_track_state == 0) {
      hall_sensor_main_track_state = 1;
  }

  hallsState[0] = hall_sensor_main_track_state;
  hallsState[1] = hall_sensor_straight_track_state;
  hallsState[2] = hall_sensor_diverging_track_state;
