...
def djikstra(
    ...
    """init"""
    time = 0
    not_visited_nodes_time = [(start_node, time)]
    predecessors = {(start_node, time): (start_node, time)}
    distances = {(start_node, time): 0}
    visited_nodes_time = []
    """loop"""
    while len(not_visited_nodes_time) > 0:
        ...
        candidate_successors[current_node] = 0  # staying in its node costs 0
...
if __name__ == "__main__":
    ...
    graphe = Graphe(G)
    agent1 = Agent("ter", graphe.get_node_with_id("C5"), graphe.get_node_with_id("C4"))
    agent2 = Agent("tgv", graphe.get_node_with_id("C7"), graphe.get_node_with_id("C8"))
